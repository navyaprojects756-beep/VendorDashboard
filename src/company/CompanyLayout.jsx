...
const GLOBAL_STYLES = `
  ...
  body.company-page { background: #FAF5FF !important; }
  ...
  .anim-shimmer {
    background: linear-gradient(90deg,#7C3AED,#A78BFA,#C4B5FD,#8B5CF6,#7C3AED);
  ...
  .s-card:hover { transform: translateY(-6px) !important; box-shadow: 0 16px 48px rgba(124,58,237,.12) !important; border-color: #DDD6FE !important }
  .w-item { transition: transform .22s ease, box-shadow .22s ease !important }
  .w-item:hover { transform: translateX(5px) !important; box-shadow: 0 4px 20px rgba(124,58,237,.08) !important }
  .c-item { transition: transform .22s ease, box-shadow .22s ease, border-color .22s ease !important }
  .c-item:hover { transform: translateY(-4px) !important; box-shadow: 0 8px 28px rgba(124,58,237,.1) !important; border-color: #DDD6FE !important }
  .v-card { transition: transform .3s ease, box-shadow .3s ease !important }
  .v-card:hover { transform: translateY(-5px) !important; box-shadow: 0 12px 36px rgba(124,58,237,.1) !important }
...
export default function CompanyLayout({ children }) {
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI',system-ui,sans-serif", background: "#FAF5FF", minHeight: "100vh", width: "100%", overflowX: "hidden" }}>
      ...
      <nav style={{
        ...
        boxShadow: scrolled ? "0 2px 20px rgba(124,58,237,.08)" : "none",
        ...
        <ul style={{ display: "flex", alignItems: "center", gap: 2, listStyle: "none", margin: 0, padding: 0 }} className="desk-nav">
          {NAV.map(n => (
            <li key={n.path}>
              <Link to={n.path} style={{
                ...
                background: pathname === n.path ? "#EDE9FE" : "transparent",
                color: pathname === n.path ? "#7C3AED" : "#475569",
                transition: "all .2s"
              }}
                onMouseEnter={e => { if (pathname !== n.path) { e.currentTarget.style.background = "#F5F3FF"; e.currentTarget.style.color = "#1E293B" }}}
          ...
          <li style={{ marginLeft: 8 }}>
            <a href="#contact" onClick={goToContact} style={{
              ...
              textDecoration: "none", background: "#7C3AED", color: "white",
              boxShadow: "0 2px 12px rgba(124,58,237,.3)", display: "block",
              transition: "all .2s"
            }}
              onMouseEnter={e => { e.currentTarget.style.background = "#6D28D9"; e.currentTarget.style.boxShadow = "0 4px 20px rgba(124,58,237,.45)" }}
              onMouseLeave={e => { e.currentTarget.style.background = "#7C3AED"; e.currentTarget.style.boxShadow = "0 2px 12px rgba(124,58,237,.3)" }}
            >Contact Us</a>
          </li>
        </ul>

        <button onClick={() => setMenuOpen(v => !v)} className="burger" style={{
          display: "none", background: "none", border: "1px solid #EDE9FE",
          borderRadius: 8, padding: "6px 10px", fontSize: "1.1rem", cursor: "pointer", color: "#7C3AED"
        }}>{menuOpen ? "✕" : "☰"}</button>
      </nav>
      ...
      {menuOpen && (
        <div style={{
          ...
          background: "rgba(255,255,255,.98)", borderBottom: "1px solid #EDE9FE",
          padding: "12px 6%", display: "flex", flexDirection: "column", gap: 4,
          boxShadow: "0 8px 24px rgba(124,58,237,.08)", animation: "fadeInDown .25s ease both"
        }}>
          {NAV.map(n => (
          ...
          <a href="#contact" onClick={goToContact} style={{ padding: "11px 16px", borderRadius: 8, textDecoration: "none", fontWeight: 600, fontSize: "0.95rem", color: "#7C3AED", background: "#EDE9FE" }}>Contact Us</a>
        </div>
      )}
      ...
      {/* FOOTER */}
      <footer style={{ background: "#0D0B1E", padding: "60px 6% 30px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 48, paddingBottom: 44, borderBottom: "1px solid rgba(255,255,255,.07)", marginBottom: 28 }} className="two-col">
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
              <div style={{ width: 34, height: 34, borderRadius: 9, background: "linear-gradient(135deg,#7C3AED,#A78BFA)", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: 900 }}>C</div>
          ...
          <div>
            ...
            <ul style={{ listStyle: "none", padding: 0, display: "flex", flexDirection: "column", gap: 10, fontSize: "0.87rem", color: "#64748B" }}>
              <li>cheritechcompany@gmail.com</li>
